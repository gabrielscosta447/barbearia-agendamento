...
<div>
    ...
    <thead class="align-bottom">
        <tr>
            <th
                class="px-6 py-3 font-bold text-left uppercase align-middle bg-transparent border-b shadow-none dark:border-white/40 dark:text-white text-xxs tracking-none whitespace-nowrap text-slate-400 opacity-70">
                Data
            </th>
            <th
                class="px-6 py-3 pl-2 font-bold text-left uppercase align-middle bg-transparent border-b shadow-none dark:border-white/40 dark:text-white text-xxs tracking-none whitespace-nowrap text-slate-400 opacity-70">
                Total de Serviços
            </th>
            <th
                class="px-6 py-3 pl-2 font-bold text-left uppercase align-middle bg-transparent border-b shadow-none dark:border-white/40 dark:text-white text-xxs tracking-none whitespace-nowrap text-slate-400 opacity-70">
    ...
    <tbody class="border-t">
        @foreach ($this->agendamentosFiltrados ?? [] as $agendamento)
            <tr>
                ...
                {{-- DATA --}}
                <td
                    class="p-2 align-middle bg-transparent {{ !$loop->last ? 'border-b dark:border-white/40' : '' }} whitespace-nowrap">
                    <div class="flex px-2">
                        <i class="text-sm leading-normal text-blue-500 ni ni-calendar-grid-58 mr-2"></i>
                        <div class="my-auto">
                ...
                {{-- AÇÕES --}}
                <td
                    class="p-2 text-sm leading-normal text-center align-middle bg-transparent {{ !$loop->last ? 'border-b dark:border-white/40' : '' }} whitespace-nowrap">

                    @if ($agendamento->deleted_at)
                        <span
                            class="bg-gradient-to-tl from-red-600 to-orange-600 cursor-pointer px-2.5 text-xs rounded-1.8 py-1.4 inline-block whitespace-nowrap text-center font-bold uppercase text-white"
                    ...
                    @else
                        <span
                            class="bg-gradient-to-tl from-emerald-500 to-teal-400 cursor-pointer px-2.5 text-xs rounded-1.8 py-1.4 inline-block whitespace-nowrap text-center font-bold uppercase text-white"
                ...
                {{-- TELEFONE --}}
                <td
                    class="p-2 mr-12 align-middle bg-transparent {{ !$loop->last ? 'border-b dark:border-white/40' : '' }} whitespace-nowrap">
                    <a
                        href="https://wa.me/{{ preg_replace('/[^0-9]/', '', 55 . $agendamento->owner->phone) }}"
                        target="_blank"
                        class="text-blue-500 text-sm underline">
                        {{ $agendamento->owner->phone ?? 'N/A' }}
                    </a>
                </td>
